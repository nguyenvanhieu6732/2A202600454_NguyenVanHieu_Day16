...
def main(dataset: str = "data/hotpot_100.json", out_dir: str = "outputs/sample_run", reflexion_attempts: int = 3) -> None:
    ...
    print(f"[bold blue]Running benchmark on {len(examples)} examples...[/bold blue]")
    
    react_records = []
    for example in track(examples, description="[cyan]ReAct Agent..."):
        react_records.append(react.run(example))
    
    reflexion_records = []
    for example in track(examples, description="[magenta]Reflexion Agent..."):
